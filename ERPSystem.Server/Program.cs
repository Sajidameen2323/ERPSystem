...
static async Task SeedDefaultAdminUser(UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager)
{
    // Ensure roles exist
    var roles = new[] { ERPSystem.Server.Common.Constants.Roles.Admin, ERPSystem.Server.Common.Constants.Roles.SalesUser, ERPSystem.Server.Common.Constants.Roles.InventoryUser };
    ...
    const string adminEmail = "admin@microbiz.com";
    ...
    if (adminUser == null)
    {
        ...
        if (result.Succeeded)
        {
            await userManager.AddToRoleAsync(adminUser, ERPSystem.Server.Common.Constants.Roles.Admin);
        }
    }
}
